        ...
        if ($row) {
            ...
            if($super_credit > 0){
                if ($usr_state == "Full") {
                    ...
                    $_SESSION['user_credit'] = $super_credit;
                } else {
                    $expdate   = date('Y-m-d h:i:sa', strtotime('+3 days'));
                }
    
    
                $stmnt = "INSERT INTO `u241746118_ems`.`adduser`(`addusr_id`, `addusr_name`,`addusr_signup_date`,`addusr_exipre`,`usraccnt_by`) VALUES ('','$usr_nickname','$signup_date','$expdate','$usercrt_sellr')";
                //print_r($stmnt);
